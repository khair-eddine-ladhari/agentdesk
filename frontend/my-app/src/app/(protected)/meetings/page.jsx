...
import { Loader2, Users, Clock, Trash2 } from "lucide-react";
...
export default function MeetingsPage() {
  ...
  async function handleDelete(meetingId) {
    if (!workspace?._id) return;
    const confirmed = window.confirm("Delete this meeting? This can't be undone.");
    if (!confirmed) return;
    ...
    setMeetings((prev) => prev.filter((m) => m._id !== meetingId)); // optimistic

    try {
      const res = await fetch(`${API_URL}/workspaces/${workspace._id}/meetings/${meetingId}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${sessionStorage.getItem("adminToken")}` },
      });
        ...
        <div className="mx-auto max-w-3xl p-6">
          ...
          {deleteError && (
            <div className="mb-3 border border-red-200 bg-red-50 p-3 text-sm text-red-700">
              Couldn't delete meeting: {deleteError}
            </div>
          )}
              ...
              {meetings.map((meeting, i) => (
                ...
                >
                  ...
                  <button
                    ...
                    className="flex h-7 w-7 shrink-0 items-center justify-center text-gray-400 transition-colors hover:bg-red-50 hover:text-red-600 disabled:opacity-50"
                  >
